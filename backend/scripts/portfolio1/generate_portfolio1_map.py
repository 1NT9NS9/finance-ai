...
CHARTS_DIR = ROOT_DIR / "charts" / "portfolio1"
INPUT_PORTFOLIO_CSV = PORTFOLIO1_DIR / "portfolio1.csv"
OUTPUT_MAP_CSV = PORTFOLIO1_DIR / "portfolio1_map.csv"
OUTPUT_SUMMARY_CSV = PORTFOLIO1_DIR / "portfolio1_summary.csv"
OUTPUT_RATE_PLOT_CSV = PORTFOLIO1_DIR / "portfolio1_map_rate_plot.csv"
# Last trades per asset (one row per symbol)
OUTPUT_LAST_WEEK_CSV = PORTFOLIO1_DIR / "portfolio1_map_last.csv"
...
def compute_trade_map_and_summary(df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if missing:
        raise ValueError(f"Missing required columns in portfolio1.csv: {missing}")
    ...
    for symbol in symbols:
        ...
        for _, r in sdf.iterrows():
            ...
            if action == "buy":
                ...
                # Non-trade row; skip mapping, though current dataset should be only buy/sell
                pass
...
def plot_cumulative_returns_per_symbol(weekly_df: pd.DataFrame) -> None:
    ...
    save to charts/portfolio1_cumulative_returns_symbols.png
    """
    ...
    ax.set_title("Total Return from Start (%) by Symbol")
    ax.set_xlabel("Week")
    ...
    fig.autofmt_xdate()
    fig.tight_layout()
    out_path = CHARTS_DIR / "portfolio1_cumulative_returns_symbols.png"
...
def plot_per_symbol(map_df: pd.DataFrame) -> None:
    ...
    for symbol, sdf in map_df.groupby("symbol"):
        ...
        ax2.set_title(f"{symbol} Capital Over Time")
        ax2.set_xlabel("Date")
        ...
        fig2.autofmt_xdate()
        fig2.tight_layout()
        out_path_equity = CHARTS_DIR / f"{symbol}_portfolio1_equity.png"
...
def main() -> int:
    ...
    # Save CSV outputs
    PORTFOLIO1_DIR.mkdir(parents=True, exist_ok=True)
