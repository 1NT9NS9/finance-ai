...
PORTFOLIO1_DIR = DATA_DIR / "portfolio1"
PORTFOLIO_CSV = PORTFOLIO1_DIR / "portfolio1.csv"
SUMMARY_CSV = PORTFOLIO1_DIR / "portfolio1_summary.csv"


def run_strategy(buy_pct: float, sell_pct: float, rsi_period: int = 6) -> Tuple[bool, Dict[str, float]]:
    """Run the pipeline generate_portfolio1.py -> generate_portfolio1_map.py and return per-symbol equity.
    ...
    cmd_gen = [
        sys.executable,
        str(ROOT_DIR / "scripts" / "portfolio1" / "generate_portfolio1.py"),
        "--buy-pct", f"{buy_pct}",
        "--sell-pct", f"{sell_pct}",
        "--rsi-period", f"{rsi_period}",
    ]
    try:
        subprocess.run(cmd_gen, check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    except subprocess.CalledProcessError as e:
        print(f"Failed to run generate_portfolio1.py ({buy_pct}, {sell_pct}): {e}")
        return False, {}

    # Run mapping/summary
    cmd_map = [
        sys.executable,
        str(ROOT_DIR / "scripts" / "portfolio1" / "generate_portfolio1_map.py"),
    ]
    try:
        subprocess.run(cmd_map, check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    except subprocess.CalledProcessError as e:
        print(f"Failed to run generate_portfolio1_map.py ({buy_pct}, {sell_pct}): {e}")
        return False, {}
    ...
    # Read summary; compute ending equity per symbol = last equity from map or cash+position from portfolio? Use summary unrealized + realized on top of initial per-asset capital (1e6).
    try:
    ...
    # Expect columns: symbol, realized_pnl, unrealized_pnl, total_pnl, ...
...
def main() -> int:
    ...
    # Buy pct from 2%..20%, Sell pct from 10%..90%
    buy_candidates = list(gridspace(0.05, 0.20, 2))
    sell_candidates = list(gridspace(0.01, 0.10, 2))
    ...
    for b, s in itertools.product(buy_candidates, sell_candidates):
        ...
        ok, results = run_strategy(b, s, rsi_period=6)
        if not ok:
            continue
    ...
    out_csv = PORTFOLIO1_DIR / "portfolio1_optimization_results.csv"
